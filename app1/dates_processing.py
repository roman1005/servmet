...
def first_last_day_of_quartal(any_day):
    ...
    if any_day.month <= 3 and any_day.month <= 31:
    ...
    elif any_day.month <= 6 and any_day.month <= 30:
    ...
    elif any_day.month <= 9 and any_day.month <= 30:
        last_day = datetime.now().replace(month=9, day=30, hour=23, minute=59, second=0, microsecond=0)
        first_day = datetime.now().replace(month=1, day=7, hour=0, minute=0, second=0, microsecond=0)

    elif any_day.month <= 12 and any_day.month <= 31:
